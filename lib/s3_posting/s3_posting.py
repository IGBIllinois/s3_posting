...
class s3_posting:
    ...
    __config = Config(
            signature_version=__signature_version,
    ...
    def get_url(self,filename,custom_param=''):
        ...
        if (custom_param != ''):
            url = self._connection.generate_presigned_url('get_object',Params = {
                        	...
                        	'Key': filename,
            self.__custom_url_var: custom_param},ExpiresIn = seconds)
        else:
            url = self._connection.generate_presigned_url('get_object',Params = {
                                'Bucket': self.__profile.get_bucket(),
                                'Key': filename},
    ...
    def connect(self):
        try:
            self._connection = boto3.client('s3',endpoint_url=self.__profile.get_endpoint_url(),
                ...
                aws_secret_access_key=self.__profile.get_secret_access_key(),config=self.__config)
